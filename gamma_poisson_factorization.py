...
#     def __init__(self, n_topics=10, batch_size=512, gamma_shape_prior=1.1,
#                  gamma_scale_prior=1.0, r=.7, hashing=False,
#                  hashing_n_features=2**12, init='k-means++', rescale_W=True,
#                  tol=1E-4, min_iter=2, max_iter=5, ngram_range=(2, 4),
#                  random_state=None, save_kl_div=False):
#
#         self.ngram_range = ngram_range
#         self.n_topics = n_topics
#         self.gamma_shape_prior = gamma_shape_prior  # 'a' parameter
...
#         H = np.ones((self.n_samples, self.n_topics))
...
#         self.kl_divergences = []
#         for iter in range(self.max_iter):
#             for i, (Ht, Vt) in enumerate(batch2(H, V, n=self.batch_size)):
#                 if i == n_batch-1:
#                     W_last = W
#                 Ht[:] = self._e_step(Vt, W, Ht)
#                 W, A, B = self._m_step(Vt, W, A, B, Ht)
#                 if self.rescale_W:
...
#                         kl_divergence = kl_div(V[ridx, :].A,
#                                                np.dot(H[ridx], W)
#                                                ).sum() / self.n_samples
#                     else:
#                         V_test = self.ngrams_count.transform(X_test)
#                         H_test = np.ones((len(X_test), self.n_topics))
#                         H_test = self._rescale_H(V_test, H_test)
#                         H_test = self._e_step(V_test, W, H_test, max_iter=100)
#                         kl_divergence = kl_div(
#                             V_test.A, np.dot(H_test, W)
#                             ).sum() / V_test.shape[0]
#
...
#         H = self._rescale_H(V, H)
#
#         for Ht, Vt in batch2(H, V, n=self.batch_size):
#             Ht[:] = self._e_step(Vt, self.W, Ht, max_iter=100)
#         return H
